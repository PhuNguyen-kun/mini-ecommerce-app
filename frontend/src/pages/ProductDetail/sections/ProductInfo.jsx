...
import { FiX } from 'react-icons/fi'; // Import thêm icon X để làm nút bỏ chọn rõ ràng hơn
...
const ProductInfo = ({ product, selectedSize, setSelectedSize, selectedColor, setSelectedColor, selectedVariant }) => {
  ...
  const handleColorClick = (colorName) => {
    if (selectedColor === colorName) {
      // Nếu đang chọn màu này rồi -> Bỏ chọn (Reset về null) -> Gallery sẽ hiện tất cả ảnh
      setSelectedColor(null);
    } else {
      // Nếu chưa chọn hoặc chọn màu khác -> Set màu mới
      setSelectedColor(colorName);
    }
  };

  // === LOGIC MỚI: XỬ LÝ CHỌN/BỎ CHỌN SIZE ===
    ...
    <div className="w-full lg:w-[384px] flex flex-col gap-px">
      ...
      <div className="py-6 sm:py-8 flex flex-col gap-2.5 items-center justify-center">
        <button
          ...
          disabled={isAdding}
          className={`w-full py-2.5 sm:py-3 flex items-center justify-center transition-colors ${addedToCart ? 'bg-green-600' : 'bg-neutral-800 hover:bg-black'
